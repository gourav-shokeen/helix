...
function buildFlowchart(nodes: FlowNode[], edges: FlowEdge[]): string {
  ...
  edges.forEach(e => {
    ...
    lines.push(`  ${e.from} ${e.style}${lbl} ${e.to}`)
  })
  return lines.join('\n')
}
...
function buildGantt(title: string, tasks: GanttTask[]): string {
  ...
  tasks.forEach(t => {
    ...
    lines.push(`  ${t.label}${alias} ${start}, ${t.duration}`)
  })
  return lines.join('\n')
}
...
// ─── Shared input style ───────────────────────────────────────────────────────
const inp: React.CSSProperties = { background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text-primary)', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, outline: 'none', padding: '0.3rem 0.5rem', width: '100%' }
...
const delBtn: React.CSSProperties = { background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 14, padding: '0 4px', lineHeight: 1, flexShrink: 0 }
const rowBox: React.CSSProperties = { background: 'var(--surface-hover)', border: '1px solid var(--border)', borderRadius: 6, padding: '0.55rem 0.75rem', display: 'flex', flexDirection: 'column', gap: 5 }
...
function FlowEditor({ nodes, edges, onChange }: { nodes: FlowNode[]; edges: FlowEdge[]; onChange: (n: FlowNode[], e: FlowEdge[]) => void }) {
  ...
  const addNode = () => onChange([...nodes, { id: `N${nodes.length + 1}`, label: 'New Node', shape: 'rect' }], edges)
  const addEdge = () => onChange(nodes, [...edges, { id: crypto.randomUUID(), from: nodes[0]?.id || 'A', to: nodes[1]?.id || 'B', label: '', style: '-->' }])
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {nodes.map((n, i) => (
        <div key={n.id} style={rowBox}>
          <div style={row}>
            {lbl('ID')}
            <input value={n.id} onChange={e => patchNode(i, { id: e.target.value })} style={{ ...inp, width: 70 }} />
            {lbl('Label')}
            <input value={n.label} onChange={e => patchNode(i, { label: e.target.value })} style={inp} />
      ...
      <button onClick={addNode} style={addBtn}>+ Add node</button>
      ...
      {edges.map((e, i) => (
        <div key={e.id} style={rowBox}>
          <div style={row}>
            {lbl('From')}
            <input value={e.from} onChange={v => patchEdge(i, { from: v.target.value })} style={{ ...inp, width: 70 }} />
            {lbl('To')}
            <input value={e.to} onChange={v => patchEdge(i, { to: v.target.value })} style={{ ...inp, width: 70 }} />
          ...
          <div style={row}>
            {lbl('Label')}
            <input value={e.label} onChange={v => patchEdge(i, { label: v.target.value })} style={inp} placeholder="optional" />
          </div>
        </div>
      ))}
      <button onClick={addEdge} style={addBtn}>+ Add edge</button>
    </div>
  )
}
...
function SeqEditor({ participants, messages, onChange }: { participants: SeqParticipant[]; messages: SeqMessage[]; onChange: (p: SeqParticipant[], m: SeqMessage[]) => void }) {
  ...
  const addP = () => onChange([...participants, { id: crypto.randomUUID(), alias: `P${participants.length + 1}`, label: 'New Person' }], messages)
  const addM = () => onChange(participants, [...messages, { id: crypto.randomUUID(), from: participants[0]?.alias || 'A', to: participants[1]?.alias || 'B', text: 'Message', style: '->>' }])
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {participants.map((p, i) => (
        <div key={p.id} style={rowBox}>
          <div style={row}>
            {lbl('Alias')}
            <input value={p.alias} onChange={e => patchP(i, { alias: e.target.value })} style={{ ...inp, width: 70 }} />
            {lbl('Label')}
            <input value={p.label} onChange={e => patchP(i, { label: e.target.value })} style={inp} />
            <button onClick={() => onChange(participants.filter((_, j) => j !== i), messages)} style={delBtn}>×</button>
          </div>
        </div>
      ))}
      <button onClick={addP} style={addBtn}>+ Add participant</button>
      ...
      {messages.map((m, i) => (
        <div key={m.id} style={rowBox}>
          <div style={row}>
            {lbl('From')}
            <input value={m.from} onChange={e => patchM(i, { from: e.target.value })} style={{ ...inp, width: 70 }} />
            {lbl('To')}
            <input value={m.to} onChange={e => patchM(i, { to: e.target.value })} style={{ ...inp, width: 70 }} />
          ...
          <div style={row}>
            {lbl('Text')}
            <input value={m.text} onChange={e => patchM(i, { text: e.target.value })} style={inp} />
          </div>
        </div>
      ))}
      <button onClick={addM} style={addBtn}>+ Add message</button>
    </div>
  )
}

function GanttEditor({ title, tasks, onTitleChange, onTaskChange, onAddTask, onRemoveTask }: {
  title: string; tasks: GanttTask[]
  onTitleChange: (v: string) => void
  onTaskChange: (i: number, p: Partial<GanttTask>) => void
  onAddTask: () => void
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={row}>
        {lbl('TITLE')}
        <input value={title} onChange={e => onTitleChange(e.target.value)} style={inp} placeholder="Chart title" />
      </div>
      {tasks.map((t, i) => (
        <div key={t.id} style={rowBox}>
          ...
          <div style={row}>
            ...
            {lbl('DUR')}
            <input value={t.duration} onChange={e => onTaskChange(i, { duration: e.target.value })} style={{ ...inp, width: 55 }} placeholder="7d" />
          </div>
        </div>
      ))}
...
function ErEditor({ entities, relations, onChange }: { entities: ErEntity[]; relations: ErRelation[]; onChange: (e: ErEntity[], r: ErRelation[]) => void }) {
  ...
  const patchField = (ei: number, fi: number, p: Partial<ErField>) => {
    const n = entities.map((x, j) => j === ei ? { ...x, fields: x.fields.map((f, k) => k === fi ? { ...f, ...p } : f) } : x)
    onChange(n, relations)
  }
  const patchRel = (i: number, p: Partial<ErRelation>) => { const n = relations.map((x, j) => j === i ? { ...x, ...p } : x); onChange(entities, n) }
  const addEntity = () => onChange([...entities, { id: crypto.randomUUID(), name: `ENTITY${entities.length + 1}`, fields: [{ id: crypto.randomUUID(), type: 'uuid', name: 'id', key: 'PK' }] }], relations)
  const addField = (ei: number) => { const n = entities.map((x, j) => j === ei ? { ...x, fields: [...x.fields, { id: crypto.randomUUID(), type: 'text', name: 'field', key: '' as const }] } : x); onChange(n, relations) }
  const addRel = () => onChange(entities, [...relations, { id: crypto.randomUUID(), from: entities[0]?.name || 'A', to: entities[1]?.name || 'B', rel: '||--o{', label: 'has' }])
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {entities.map((e, ei) => (
        <div key={e.id} style={{ ...rowBox, gap: 6 }}>
          <div style={row}>
            {lbl('NAME')}
            <input value={e.name} onChange={v => patchEntity(ei, { name: v.target.value })} style={{ ...inp, fontWeight: 700 }} />
            <button onClick={() => onChange(entities.filter((_, j) => j !== ei), relations)} style={delBtn}>×</button>
          </div>
          {e.fields.map((f, fi) => (
            <div key={f.id} style={{ ...row, paddingLeft: 8 }}>
              ...
              <button onClick={() => { const n = entities.map((x, j) => j === ei ? { ...x, fields: x.fields.filter((_, k) => k !== fi) } : x); onChange(n, relations) }} style={delBtn}>×</button>
            </div>
          ))}
          <button onClick={() => addField(ei)} style={{ ...addBtn, fontSize: 10, padding: '0.2rem' }}>+ field</button>
        </div>
      ))}
        ...
        <div key={r.id} style={rowBox}>
          <div style={row}>
            ...
            <select value={r.rel} onChange={e => patchRel(i, { rel: e.target.value })} style={{ ...inp, width: 90 }}>
              <option value="||--o{">one-to-many</option>
              <option value="||--||">one-to-one</option>
              <option value="}o--o{">many-to-many</option>
              <option value="||--o|">one-to-zero-one</option>
            </select>
      ...
      <button onClick={addRel} style={addBtn}>+ Add relation</button>
    </div>
  )
}
...
export function DiagramModal({ onInsert, onClose, initialDsl, mode = 'insert' }: DiagramModalProps) {
  ...
  // Fix SVG sizing + colors after render
  useEffect(() => {
    ...
    if (diagramType === 'Gantt') {
      ...
      // Read mermaid's natural px width/height → build viewBox → remove attrs
      // → set width:100% so it scales proportionally to fit the panel.
  ...
  const initFromDsl = useCallback((dsl: string) => {
    setSvg('')
    const type = detectType(dsl)
    setDiagramType(type)
    setSyntax(dsl)
    if (type === 'Gantt') {
      const { title, tasks } = parseGantt(dsl)
      setGanttTitle(title)
      setGanttTasks(tasks)
    }
    setTimeout(() => {
  ...
  const handleTemplate = (key: string) => {
    setSvg('')
    setError('')
    initFromDsl(TEMPLATES[key])
  }

  const handleDslChange = (val: string) => {
    setSyntax(val)
    setDiagramType(detectType(val))
    if (debounceRef.current) clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(() => renderDiagram(val), 400)
  }

  // Visual editor change handlers
  const handleFlowChange = (n: FlowNode[], e: FlowEdge[]) => {
    setFlowNodes(n); setFlowEdges(e)
    updateFromVisual(buildFlowchart(n, e))
  }
  const handleSeqChange = (p: SeqParticipant[], m: SeqMessage[]) => {
    setSeqParticipants(p); setSeqMessages(m)
    updateFromVisual(buildSequence(p, m))
  }
  const handleGanttTitleChange = (v: string) => {
    setGanttTitle(v); updateFromVisual(buildGantt(v, ganttTasks))
  }
  const handleGanttTaskChange = (i: number, p: Partial<GanttTask>) => {
    const updated = ganttTasks.map((t, j) => j === i ? { ...t, ...p } : t)
    setGanttTasks(updated); updateFromVisual(buildGantt(ganttTitle, updated))
  }
  const handleAddGanttTask = () => {
    const t: GanttTask = { id: crypto.randomUUID(), label: 'New Task', alias: `t${ganttTasks.length + 1}`, startType: 'date', startDate: new Date().toISOString().slice(0, 10), afterAlias: '', duration: '3d', section: ganttTasks[ganttTasks.length - 1]?.section || 'Phase 1' }
    const updated = [...ganttTasks, t]; setGanttTasks(updated); updateFromVisual(buildGantt(ganttTitle, updated))
  }
  const handleRemoveGanttTask = (i: number) => {
    const updated = ganttTasks.filter((_, j) => j !== i); setGanttTasks(updated); updateFromVisual(buildGantt(ganttTitle, updated))
  }
  const handleErChange = (e: ErEntity[], r: ErRelation[]) => {
    setErEntities(e); setErRelations(r)
    updateFromVisual(buildEr(e, r))
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400 }} onClick={onClose}>
      <div className="helix-fade-in" onClick={e => e.stopPropagation()}
          ...
          <div style={{ display: 'flex', gap: '0.4rem', marginLeft: 'auto' }}>
            {Object.keys(TEMPLATES).map(name => (
              <button key={name} onClick={() => handleTemplate(name)}
                style={{ background: 'var(--surface-hover)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, padding: '0.25rem 0.6rem' }}>
                {name}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: '0 0 42%', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border)', minWidth: 0, overflow: 'hidden', position: 'relative' as const }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              {hasVisualEditor && (
                <div style={{ fontSize: 10, color: 'var(--accent)', padding: '0.4rem 0.75rem', background: 'var(--accent-dim)', borderRight: '1px solid var(--border)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  ✦ Visual Editor
                </div>
              )}
              <div style={{ fontSize: 10, color: 'var(--text-muted)', padding: '0.4rem 0.75rem', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                {hasVisualEditor ? 'DSL (auto-synced)' : 'Mermaid DSL'}
              </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, maxWidth: '100%' }}>
              ...
              {hasVisualEditor && (
                <div style={{ overflowY: 'auto', overflowX: 'hidden', flexShrink: 0, maxHeight: '58%', padding: '0.75rem', borderBottom: '1px solid var(--border)' }}>
                  ...
                  {diagramType === 'Gantt' && (
                    <GanttEditor title={ganttTitle} tasks={ganttTasks}
                      onTitleChange={handleGanttTitleChange}
                      onTaskChange={handleGanttTaskChange}
                      onAddTask={handleAddGanttTask}
                      onRemoveTask={handleRemoveGanttTask}
                    />
                  )}
          ...
          <div style={{ flex: '0 0 58%', display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden', position: 'relative' as const }}>
            <div style={{ fontSize: 10, color: 'var(--text-muted)', padding: '0.4rem 0.75rem', borderBottom: '1px solid var(--border)', textTransform: 'uppercase', letterSpacing: '0.07em', flexShrink: 0 }}>
              Preview
            </div>
            {/* ── CHANGED: flex centering + overflow scroll replaces textAlign/whiteSpace trick ── */}
            <div style={{
              flex: 1,
              overflowY: 'auto',
              overflowX: 'auto',
              ...
              justifyContent: diagramType === 'Sequence' ? 'flex-start' : 'center',
            }}>
              {error
        ...
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', padding: '0.65rem 1rem', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
          <button onClick={onClose} style={{ background: 'var(--surface-hover)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, padding: '0.4rem 0.9rem' }}>
            Cancel
          </button>
